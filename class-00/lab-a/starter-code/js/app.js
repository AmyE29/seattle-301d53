...
function displayPics(){
  ...
  // PUT YOUR RESPONSE IN THIS COMMENT  Since let was defined inside the function it has to be console.logged within the function to be a valid variable.
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} votes in ${allProducts[i].views} view`);
    }
  }
